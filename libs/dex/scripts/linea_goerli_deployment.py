from brownie import MockERC20, MockUSDERC20, MockWETH, MockWBTCERC20, Multicall3, XfaiLibrary, InfinityNFTPeriphery, XfaiV0Periphery03, XfaiINFT, XfaiV0Core, XfaiFactory, accounts, chain, interface
...
baseURI = 'some_link/'
goerli_weth = '0x2C1b868d6596a18e32E61B901E4060C872647b6C'  # goerli weth address

def main(wait=False):
    if sys.argv[-1] == 'linea-goerli':
    ...
    xfai_factory = local_account.deploy(XfaiFactory, local_account, weth_address)
    inft =  local_account.deploy(XfaiINFT, xfai_factory, weth_address, xfit, 1e19, 2)
    inft.premint(
        [local_account, local_account], 
        [5e17, 5e17], {'from': local_account}
    )
    core = local_account.deploy(XfaiV0Core, xfai_factory, inft, weth_address, lp_fee, inft_fee)
    xfai_factory.setXfaiCore(core, {'from': local_account})
    periphery = local_account.deploy(XfaiV0Periphery03, xfai_factory, weth_address)
    infinity_periphery = local_account.deploy(InfinityNFTPeriphery, xfai_factory, xfai_factory, inft, xfit, weth_address)
