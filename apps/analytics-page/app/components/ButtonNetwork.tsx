...
import { IconArrowDown } from '@xfai-labs/ui-components';
import { FunctionComponent, HTMLProps, ReactElement, useMemo } from 'react';
...
const ButtonNetwork: FunctionComponent<Props> = ({ icon, dropdown, className, ...props }) => {
  const path = usePathname();

  const selectedNetwork = useMemo(
    () => Networks.find((n) => path?.includes(n.analyticsURL)) ?? Networks[0],
    [path],
  );
  ...
  return (
    <button className={buttonClasses} {...props}>
      ...
      <span className="hidden !leading-none lg:block">{selectedNetwork.name}</span>
      {dropdown && (
